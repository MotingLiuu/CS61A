...
```py
obj = MyClass(return_dict=False, another_param=42)
# kwargs = {"return_dict": False, "another_param": 42}
```

- `**kwargs` 是一种可变参数，用于捕获任意数量的关键字参数（键值对）。
- 在方法中，`kwargs` 是一个字典，包含所有未明确指定的关键字参数。
