...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    paragraphs_true = [para for para in paragraphs if select(para)]
    return paragraphs_true[k] if len(paragraphs_true)-1 >= k else ""
    # END PROBLEM 1
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    typed_index = min(len(source_words), len(typed_words))
    acc = [1 if typed_words[i] == source_words[i] else 0 for i in range(typed_index)]
    if sum(acc) != 0:
        return sum(acc) / len(typed_words) * 100
...
def report_progress(typed, source, user_id, upload):
    ...
    message = {'id': user_id, 'progress': count / len(source)}
    upload(message)
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = [[timestamps_per_player[id][i+1]-timestamps_per_player[id][i] for i in range(len(timestamps_per_player[id])-1)] for id in range(len(timestamps_per_player))]  # You may remove this line   
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    fastest_words = [[] for id in player_indices]
    for i in word_indices:
        times_word = [get_time(times, id, i) for id in player_indices]
        fastestPlayer = times_word.index(min(times_word))
        fastest_words[fastestPlayer].append(words[i])
